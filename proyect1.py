...
# • Crea Introducción (≥ 9 pág.), Bases Teóricas e Hipótesis.
# • Exporta las secciones a un archivo .docx.
# ---------------------------------------------------------

# ⚠️ `st.set_page_config` DEBE ser la primera llamada Streamlit
...
def build_paragraph(core: str) -> str:
    """Garantiza párrafos ≥ PARA_WORDS palabras y ≤ 10 líneas aprox."""
    base = core.strip()
...
def generate_introduction(title: str, objective: str, min_words: int = 4500) -> str:
    """Construye la introducción completa conforme a la estructura requerida."""

    title_l = title.lower().strip()

    # Bloques base (12 párrafos)
    blocks = [
    ...
    # Aumentar tamaño hasta ≥ min_words
    i = 0
...
def build_docx(intro: str, bases: str, hyps: str) -> bytes:
    ...
    for p in intro.splitlines():
        doc.add_paragraph(p)
    doc.add_page_break()

    doc.add_heading("Bases Teóricas", level=2)
    for p in bases.splitlines():
        doc.add_paragraph(p)
    doc.add_page_break()

    doc.add_heading("Hipótesis", level=2)
    for p in hyps.splitlines():
        doc.add_paragraph(p)
...
if st.sidebar.button("Generar Secciones"):
    ...
    st.download_button("Descargar DOCX", data=docx_data, file_name=f"secciones_tesis_{date.today()}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document")
